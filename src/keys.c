...
static const int scancodeCount = 37;

static const char scancode[] = {
	0x1E, 0x30, 0x2E, 0x20, 0x12, 0x21, 0x22, 0x23, 0x17, 0x24, 0x25, 0x26, 0x32,
	0x31, 0x18, 0x19, 0x10, 0x13, 0x1F, 0x14, 0x16, 0x2F, 0x11, 0x2D, 0x15, 0x2C,
	0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x35
};
...
void drawChar(char c,int x, int y, int color, int scale, char *buffer) {
	int i, j, si, sj, lin = 0;
	char pos;
	for (i = 0; i < charHeight*scale;) {
		for (si = 0; si < scale; si++) {
			pos = 1;
			for (j = charWidth*scale - 1; j >= 0;) {
				for (sj = 0; sj < scale; sj++) {
					if ((table[charHeight*c+lin] & pos) != 0) {
						set_pixelb(x+j, y+i, color, buffer);
					}
					j--;
				}
				pos = pos << 1;
...
void drawString(char* str,int x, int y, int color, int scale, char *buffer) {
	while (*str != 0) {
		drawChar(*str, x, y, color, scale, buffer);
		str++;
		x += charWidth * scale;
	}
}
